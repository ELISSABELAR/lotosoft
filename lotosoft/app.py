...
def run(grid_type, batch_size, force_page):
    ...
    for offset in range(pages):
        ...
        if soup is None:
            if not force_page: state.advance()
            continue
        ...
        for ref in refs:
            ...
            if stats is None:
                logger.info(f"     → stats: skipped (missing cote)")
                n_skip += 1
    ...
    logger.info(f"[{grid_type}] done: total={n_total} stats={n_stats} "
                f"results={n_results} skip={n_skip} files={count_files(grid_type)}")
    return {"total": n_total, "stats_ok": n_stats,
            "results_ok": n_results, "skipped": n_skip,
