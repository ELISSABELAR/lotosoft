...
def _path(grid_type, season, record_id):
...
def build_record(ref, stats, results):
    a_map = {m.num: m for m in (stats.matches   if stats   else [])}
    b_map = {m.num: m for m in (results.matches if results else [])}
    nums  = sorted(set(list(a_map) + list(b_map)) or [])
    return {
        ...
        "matches": [_build_match(n, a_map.get(n), b_map.get(n)) for n in nums],
        "ranks": [
...
def save_record(ref, stats, results):
    p   = _path(ref.grid_type, ref.season, ref.record_id)
    new = build_record(ref, stats, results)

    # Upsert : ne pas écraser les données existantes avec None
    if p.exists():
        try:
            old     = json.loads(p.read_text(encoding="utf-8"))
            old_map = {m["num"]: m for m in old.get("matches", [])}
