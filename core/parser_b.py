"""Parser B — post-match results page."""
import logging, re
...
def _parse_results_table(table):
    ...
    for row in table.find_all("tr"):
        ...
        if len(cells) == 1:
            ...
            if txt and not txt.isdigit(): date_str = txt
            continue
...
def parse_results_page(soup, record_id, url, grid_size):
    ...
    if not page.matches: return None
    idx       = tables.index(hist)
    remaining = tables[idx + 1:]
    if remaining:        page.ranks = _parse_ranks_table(remaining[0])
    if len(remaining) > 1: page.stats = _parse_stats(remaining[1:])
    return page

def fetch_results(session, url, record_id, grid_size):
    from .utils import fetch as _fetch
    soup = _fetch(session, url, delay=DELAY_RESULTS)
