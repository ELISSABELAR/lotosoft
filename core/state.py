...
class RunState:
    ...
    def _load(self):
        ...
        try:
            ...
            logger.info(
                f"[{self.grid_type}] state: page {self.next_page}/{self.total_pages or '?'}, "
    ...
    def mark(self, rid):     self.done.add(rid)
    def is_done(self, rid):  return rid in self.done
    def advance(self):
        self.next_page += 1
        if self.total_pages > 0 and self.next_page > self.total_pages:
            self.complete = True
            logger.info(f"[{self.grid_type}] complete!")
